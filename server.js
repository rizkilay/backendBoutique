...
const pool = new Pool({
  connectionString:
    ...
    'postgresql://postgres:Takya@5#Moi@db.hkvujwkxxnivjgjjsdja.supabase.co:5432/postgres',
...
app.get('/api/summary', async (req, res) => {
  ...
  try {
    ...
    const recentResult =
      await pool.query('SELECT * FROM cotisations ORDER BY date DESC LIMIT 50');

    const withdrawalsResult =
      await pool.query('SELECT * FROM cotisation_withdrawals ORDER BY date DESC LIMIT 50');
  ...
  try {
    ...
    if (Array.isArray(cotisations)) {
      ...
      for (const c of cotisations) {
        ...
        await client.query(`
          ...
          (id,amount,date,note,source,category,partner_id)
    ...
    if (Array.isArray(withdrawals)) {
      ...
      for (const w of withdrawals) {
        ...
        await client.query(`
          INSERT INTO cotisation_withdrawals
          (id,cotisation_id,amount,date,motif,source)
          VALUES ($1,$2,$3,$4,$5,$6)
        `, [
          w.id,
          w.cotisation_id,
    ...
    res.json({
      message: 'Cotisation sync successful'
    });
